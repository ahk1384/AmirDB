    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
                ...
                System.out.println(ui.printlnInfo("Request: " + request.getType() + " - " + request.getQuery()) + " - From " + socket.getInetAddress() + ":" + socket.getPort());
                if (request.getType() == MessageType.AUTH) {
                    if (request.getQuery().equals(Pass) || Pass == null) {
                        out.writeObject(new Response(true, "Accepted"));
                        System.out.println(ui.printlnSuccess("New USer Logged in ! "));
                    } else {
                        out.writeObject(new Response(false, "Wrong Password"));
                        System.out.println(ui.printlnError("Wrong Password !"));
                    }
                } else if (request.getType() == MessageType.EXIT) {
                    System.out.println(ui.printlnError("Client disconnected -" + socket.getInetAddress() + ":" + socket.getPort()));
